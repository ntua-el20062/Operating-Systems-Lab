                             ...
                             struct page **res_page)
{
    ...
    *res_page = NULL;
    ...
    for (i = 0; i < npages; i++) {
        page = ext2_get_page(dir, i, 0);  // Get the i-th page of the directory
        if (IS_ERR(page))
            return ERR_CAST(page);

        kaddr = page_address(page);  // Get the kernel virtual address of the page

        /* Iterate through directory entries on the page */
        de = (ext2_dirent *)kaddr;
        kaddr += ext2_last_byte(dir, i) - reclen;
        while ((char *)de <= kaddr) {
            if (ext2_match(namelen, name, de)) {
                *res_page = page;
                return de;  // Entry found, return the entry
            }
            de = ext2_next_entry(de);  // Move to the next directory entry
        }

        ext2_put_page(page);  // Release the page as it is no longer needed
    }
